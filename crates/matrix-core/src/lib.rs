...
pub use dry_run::{
    create_dry_run_plan, dry_run_plan_response_data, DryRunPermissionSnapshot, DryRunPlan,
    DryRunStep,
};
...
const DRY_RUN_MIGRATION_SQL: &str = dry_run::MIGRATION_SQL;
...
pub fn ensure_schema(db_path: &Path) -> Result<(), CoreError> {
    ...
    for (version, name, migration_sql) in [
        ...
        (4_i64, "0004_dry_run_plans", DRY_RUN_MIGRATION_SQL),
    ] {
...
mod tests {
    ...
    fn sqlite_schema_is_idempotent() {
        ...
        .expect("read third migration backups")
        .filter_map(Result::ok)
        .any(|entry| entry
            .file_name()
            .to_string_lossy()
            .starts_with("app.sqlite3.before-migration-v0004-")));
    }
